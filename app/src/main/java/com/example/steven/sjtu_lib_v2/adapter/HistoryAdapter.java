 ...
 */
public class HistoryAdapter extends ArrayAdapter<Element> {

    @Override
    public View getView(int position, View convertView, ViewGroup parent) {
        Element complete_info=getItem(position);
        Elements elements_td1=complete_info.getElementsByClass("td1");
        String book_name=elements_td1.get(2).text();
        String author=elements_td1.get(1).text();
        String return_day=elements_td1.get(4).text();
        String return_time=elements_td1.get(7).text();
    ...
    public HistoryAdapter(Context context, int resource, List<Element> objects) {
